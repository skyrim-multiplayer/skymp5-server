  ...
  if (it == forms.end()) {
    if (formId < 0xff000000) {
      if (LoadForm(formId, optionalOutTrace)) {
        ...
        if (it != forms.end()) {
          ctx.AddMessage("found after successful LoadForm");
          return it->second;
        }
        ctx.AddMessage("not found after successful LoadForm");
        return kNullForm;
      } else {
        ctx.AddMessage("LoadForm returned false");
      }
    }
    ctx.AddMessage("not found");
    return kNullForm;
  }

  ctx.AddMessage("found");
  return it->second;
}
